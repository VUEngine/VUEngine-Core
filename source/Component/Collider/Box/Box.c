...
void Box::print(int32 x, int32 y)
{
	...
	Printer::int32(__METERS_TO_PIXELS(rightBox.x0 + this->transformation->position.x), x + 2, y, NULL);
	Printer::text("-" , x + 6, y, NULL);
	Printer::int32(__METERS_TO_PIXELS(rightBox.x1 + this->transformation->position.x), x + 8, y++, NULL);

	Printer::text("Y:             " , x, y, NULL);
	Printer::int32(__METERS_TO_PIXELS(rightBox.y0 + this->transformation->position.y), x + 2, y, NULL);
	Printer::text("-" , x + 6, y, NULL);
	Printer::int32(__METERS_TO_PIXELS(rightBox.y1 + this->transformation->position.y), x + 8, y++, NULL);
