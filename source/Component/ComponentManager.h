...
singleton class ComponentManager : Object
{
	...
	static void removeComponent(GameObject owner, Component component);
	...
	static void addComponents(GameObject owner, ComponentSpec** componentSpecs, uint32 componentType);

	/// Remove components from the specified owner.
	/// @param owner: Object to which the components attach to
	/// @param componentType: Type of components to remove
	static void removeComponents(GameObject owner, uint32 componentType);
	...
	static Component getComponentAtIndex(GameObject owner, uint32 componentType, int16 componentIndex);
	...
	static VirtualList getComponents(GameObject owner, uint32 componentType);
	...
	static bool getComponentsOfClass(GameObject owner, ClassPointer classPointer, VirtualList components, uint32 componentType);
	...
	static uint16 getComponentsCount(GameObject owner, uint32 componentType);
	...
	void propagateCommand(int32 command, GameObject owner, ...);

	/// Retrieve the number of components belonging to the provided owner.
	/// @param owner: Object to which the components attach to
	/// @return Number of components belonging to the provided owner
	uint16 getCount(GameObject owner);
	...
	virtual Component createComponent(GameObject owner, const ComponentSpec* componentSpec);

	/// Destroy the provided component.
	/// @param owner: Object to which the component will attach to
	/// @param component: Comoponent to destroy
	virtual void destroyComponent(GameObject owner, Component component) ;

	/// Retrieve a list with the components of the provided type belonging to the provided owner.
	/// @param owner: Object to which the components attach to
	/// @return Linked list of components of the type provided that attach to the provided owner
	VirtualList doGetComponents(GameObject owner, VirtualList components);
	...
	virtual bool isAnyVisible(GameObject owner);
}
