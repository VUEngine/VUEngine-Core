...
void OptionsSelector_setOptions(OptionsSelector this, VirtualList optionsNames)
{
	...
	if (0 < VirtualList_getSize(optionsNames))
	{
		...
		this->currentOption = this->currentPage ? VirtualList_begin(__UPCAST(VirtualList, VirtualNode_getData(this->currentPage))) : NULL;
	}
...
void OptionsSelector_selectNext(OptionsSelector this)
{
	...
	if (this->currentOption)
	{
		...
		if (!this->currentOption)
		{
			...
			this->currentOption = VirtualList_begin(__UPCAST(VirtualList, VirtualNode_getData(this->currentPage)));
...
void OptionsSelector_selectPrevious(OptionsSelector this)
{
	...
	if (this->currentOption)
	{
		...
		if (!this->currentOption)
		{
			...
			this->currentOption = VirtualList_end(__UPCAST(VirtualList, VirtualNode_getData(this->currentPage)));
...
void OptionsSelector_showOptions(OptionsSelector this, int x, int y)
{
	...
	if (this->currentPage && 0 < VirtualList_getSize(__UPCAST(VirtualList, VirtualNode_getData(this->currentPage))))
	{
		this->x = 0 <= x && x <= __SCREEN_WIDTH >> 3 ? x : 0;
		this->y = 0 <= y && y <= __SCREEN_HEIGHT >> 3 ? y : 0;

		ASSERT(this->currentPage, "showOptions: currentPage");
		VirtualNode node = VirtualList_begin(__UPCAST(VirtualList, VirtualNode_getData(this->currentPage)));
...
static void OptionsSelector_printSelectorMark(OptionsSelector this, char* mark)
{
	...
	if (this->currentPage)
	{
		...
		int indexOption = this->currentOptionIndex - this->currentPageIndex * VirtualList_getSize(__UPCAST(VirtualList, VirtualList_front(this->pages)));
